...
counts_p<- read.delim(" /Isoform_DEI/Isoforms_DEI_switch_DESeq2/Counts_MG_switchisoform.txt", header=T)
...
c_clean<-filter(counts_p_def, cpm_i2>0.143  & cpm_ni2>0.354  & cpm_i3>0.122 & cpm_ni3>0.281 ) #1394 isoformas quedan
##c_clean2<-filter(counts_p_def, cpm_mg1>0.18  & cpm_sg1>0.23  & cpm_mg3>0.26  & cpm_sg3>0.11 ) # 1749 isoformas quedan


write.table(c_clean, file = " /DESeq2/All_isoforms_filtered_by_cpm_more_than_one_transcript_ctrl_inf_MG.txt",quote = FALSE, sep="\t", row.names = FALSE, col.names = T)

########################################################################################################
iso_filter<- read.delim(" /DESeq2/All_isoforms_filtered_by_cpm_more_than_one_transcript_ctrl_inf_MG.txt", header=T)
...
dim(dataset3) #1394
write.table(data_counts," /DESeq2/DESEQ2_norm_counts_MG_Ctrl_vs_Infec.txt", quote = F,row.names = T, col.names = T,sep = "\t")
...
write.table(isoforms_DEI, file = " /DESeq2/Isoforms_DEI_Ctrol_vs_Infec_MG_0.05.txt",quote = FALSE, sep="\t", row.names = T , col.names = T)
...
load("/isoform/RData/analisis_2.RData")
...
analycedSwitchList_MG <- isoformSwitchAnalysisPart1(
  switchAnalyzeRlist   = dataList_MG2,
  genomeObject = Agam,
...
DUI_mg<-analycedSwitchList_MG$isoformSwitchAnalysis ### 13 isoformas son DUI correspondientes a 4 genes
write.table((DUI_mg), file = " /Iso_usage/Isoforms_DUI_Ctrol_vs_Infec_MG_0.05.txt",quote = FALSE, sep="\t", row.names = T , col.names = T)
